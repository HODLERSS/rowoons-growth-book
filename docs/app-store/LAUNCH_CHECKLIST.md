...
- App Store: the iOS build hides Google sign-in (email link only) until Sign in with Apple is wired, which satisfies guideline 4.8 by omission. The Sign in with Apple code (native plugin + web OAuth) is already in the app; once the Apple provider is configured, build with `NEXT_PUBLIC_APPLE_SIGNIN=1` to show Apple and Google together. Steps in `READINESS.md`.
