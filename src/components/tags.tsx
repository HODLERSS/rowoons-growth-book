...
export function SeverityTag({ severity }: { severity: Severity }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 text-[0.75rem] font-bold uppercase tracking-[0.08em]", cls)}>
...
export function DifficultyTag({ difficulty }: { difficulty: Difficulty }) {
  const { t } = useLanguage();
  return (
    <span className="inline-flex h-6 items-center rounded-full bg-hover px-2 text-[0.75rem] font-semibold text-muted-foreground">
...
export function CategoryLabel({ category, className }: { category: MilestoneCategory; className?: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1.5 text-[0.75rem] font-semibold text-muted-foreground", className)}>
